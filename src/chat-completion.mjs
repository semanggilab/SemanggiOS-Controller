...
// Gateway fork menutup run yang gagal sebelum menghasilkan apa pun (kuota
// provider, billing, galat model) dengan SATU pesan asisten berkalimat tetap —
// terukur 2026-09-11 pada cerebras/qwen-3-8-27b dan groq/qwen-medium: teks itu
// BUKAN jawaban, ia vonis. Tanpa pemeriksaan ini poller menyegelnya DONE dan
// operator membaca kegagalan provider seolah-olah itu balasan brain.
const RUN_FAILURE_REPLY = /^the agent run failed before producing a reply\.?$/i;

function isRunFailureReply(text) {
  return RUN_FAILURE_REPLY.test(String(text ?? "").trim());
}
...
export function createChatCompletion({ repos, events, runtime, log, now = () => Date.now(), config = {} }) {
  ...
  async function completeOne(row) {
    if (row.status === "RUNNING" && row.gateway_session_ref) {
      ...
      if (reply) {
        const text = extractReplyText(reply.content);
        if (isRunFailureReply(text)) {
          // Vonis gateway, bukan jawaban: FAILED dengan teks vonisnya sebagai
          // error — UI menuliskannya merah, bukan merender markdown.
          await repos.chatMessages.updateDelivery(row.id, { status: "FAILED", error: text });
          ...
          await events.append({
            ...
            actor: "controller",
            payload: { messageId: row.id, reason: "gateway-run-failed", gateway: row.gateway_session_ref },
          });
          return "failed";
        }
        await repos.chatMessages.updateDelivery(row.id, { status: "DONE", content: text });
        await repos.chatSessions.touch(row.session_id);
        await events.append({
