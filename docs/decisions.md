...
## D84 — Context window dan max output di Model Map: dua RPC, satu baris, nol jalur tulis

**Pertanyaan operatornya:** input "Context window" di form Add/Edit Brain — bisakah ia mengontrol `contextWindow` per model di OpenClaw? **Jawabannya tidak, dan alasannya struktural, bukan soal usaha.**

**Granularitas Brain bukan granularitas model.** `contextWindow` adalah properti MODEL di openclaw.json; Brain adalah pasangan (model, level). Empat model punya lebih dari satu Brain — `zai/glm-5.2` (max/high/low), `claude-code/claude-code` (opus/sonnet/batch), `google/gemini-3.1-flash-lite` (high/medium), `groq/qwen/qwen3.6-27b` (high/medium) — jadi tiga Brain glm-5.2 akan menulis tiga nilai ke satu field, dan tidak ada jawaban yang benar untuk "siapa menang". Yang punya granularitas persis sama dengan openclaw adalah **Model Map** (D66), yang memang sudah jadi pemilik pasangan (provider, model).

**Kolom Brain-nya juga bukan kontrol.** `brains.context_window_tokens` lahir di D63 sebagai FAKTA kuota per Brain, bernilai `null` untuk seluruh 19 baris, dan tidak dibaca satu keputusan pun. Menjadikannya perintah berarti satu kolom kadang berarti "yang saya ukur" dan kadang "yang saya paksakan — kebohongan berbentuk sama dengan yang D17 sudah bayar.

**Dan controller memang tidak bisa menulis ke sana.** Mount-nya `controller/`, `controller-src/`, `openclaw/workspaces` — direktori config gateway tidak termasuk. Dari 16 operasi WS yang benar-benar lewat dalam 24 jam, yang menyentuh config hanya `config.get`. Menambah jalur tulis berarti salah satu dari: penulis KETIGA ke openclaw.json (AgentOS sudah menulisnya langsung, D28/D32; §5.4 mencatat apa yang terjadi saat uid-nya salah), atau RPC tulis yang harus diverifikasi ada DAN scope device-nya mengizinkan (D70: device hanya admin/read/write; `operator.*` sudah ditolak 403).

**Keputusan: tampilkan, jangan sunting.** Model Map mendapat dua kolom BACA — `Context window` dan `Max output`. Sebelum ini angka itu tidak muncul di mana pun di UI: 13 model disetel lewat SSH dan tidak ada satu layar pun yang memperlihatkannya.

**Sumbernya bukan `models.list`, dan itu diukur, bukan diasumsikan** (aturan 4 §4.1). Satu entri `models.list` di 2026.8.2 berisi persis `{id, provider, name, reasoning, available}` — tidak ada satu pun angka batas. Keduanya tinggal di `models.providers[].models[]` milik config gateway, dibaca lewat `config.get` (`runtime.modelLimits()`). Cabang `resolved` dibaca lebih dulu, `config` sebagai fallback: yang menentukan perilaku run adalah nilai EFEKTIF setelah katalog bawaan digabung — glm-5.1 memperlihatkan bedanya, tidak menyetel `maxTokens` sama sekali di file tetapi berjalan pada 131072 dari katalog. Default provider-level ditiru juga (`m.maxTokens ?? provider.maxTokens`), karena itulah aturan yang skema openclaw.json nyatakan.

**Cache, bukan RPC per page load.** Kolom `context_window`/`max_tokens` menempel di `gateway_models`, diisi saat "Refresh Models" — aturan yang sama dengan `GET /gateway/models`. Kegagalan sisi batas tidak menjatuhkan refresh daftar model; ia jatuh ke daftar kosong dan kolomnya tinggal NULL. Migrasi tanpa backfill: nilainya hanya diketahui gateway, dan menebaknya berarti menampilkan angka yang tidak pernah diukur.

**Batas gateway MENEMPEL pada baris yang sudah berdiri, tidak pernah membuat baris baru.** Sebuah model yang diiklankan gateway tetapi tanpa baris resources maupun thinking-levels bukan baris Model Map; menampilkannya akan membuat halaman ini mengklaim kebijakan yang tidak pernah ada. `null` dipertahankan sebagai `null` — "gateway tidak melaporkannya" dan "nol" adalah dua jawaban berbeda, dan 0 di kolom context window terbaca sebagai model yang lumpuh.

**Test:** 561 (dari 558). Dipaku: batas menempel tanpa melahirkan baris; baris tanpa padanan cache tetap tampil dengan `null`; pemanggil dua-argumen lama tidak berubah perilaku.
