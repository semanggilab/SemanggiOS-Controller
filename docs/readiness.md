...
| Alibaba Model Studio (provider `aliyuncs`, OpenAI-compatible) | Endpoint workspace `ws-pyewjaac2j09a9j5.ap-southeast-1.maas.aliyuncs.com/compatible-mode/v1` (api `openai-completions`) terkonfigurasi di gateway; lima model — qwen3.7-flash, qwen3.7-max, qwen3.8-27b, qwen3.8-flash, qwen3.8-max — semuanya `available` di daftar hidup gateway (refresh 2026-09-06). Probe level thinking per model: SEMUA menerima `off\|minimal\|low\|medium\|high` dan MENOLAK `max`+`adaptive` (status error saat start, pola sama di kelimanya). Lima Brain terdaftar: `alibaba-qwen3-8-max` (critical), `alibaba-qwen3-7-max` (critical), `alibaba-qwen3-8-27b` (normal), `alibaba-qwen3-8-flash` (low), `alibaba-qwen3-7-flash` (low) — semua thinking=`high` (level terukur tertinggi), effortMode `guaranteed` atas permintaan operator. Uji otak qwen3.8-max menyelesaikan run nyata: `stopReason=stop`, 4,8 dtk. Agen probe: `sem-workspaces-probe-alibaba-*` di workspace `workspaces/probe/alibaba` |
| Cerebras `qwen-3.8-27b` — terdaftar, TERTAHAN billing (402) | Provider `cerebras` ditambahkan operator lewat AgentOS (`api.cerebras.ai`, openai-completions); model `available` di daftar hidup gateway. Probe 2026-09-06: ketujuh kandidat level dispatch diterima, tetapi SETIAP run nyata gagal **HTTP 402 (billing)** dari `api.cerebras.ai` — tak satu level terukur end-to-end. Kosakata level dikonfirmasi validasi gateway sendiri: `off\|minimal\|low\|medium\|high` (pesan penolakan eksplisit untuk `max`/`adaptive`). Brain `cerebras-qwen-3-8-27b` (BRN-3CA78DD7, normal, thinking=`high`, effort `guaranteed`) tetap terdaftar dengan bukti 402 di effortEvidence; agen `sem-workspaces-probe-cerebras-cerebras-qwen-3-8-27b` di `workspaces/probe/cerebras`. **Tindak lanjut operator:** isi kredit/paket Cerebras, lalu probe ulang — sampai itu, dispatch ke Brain ini akan gagal 402 |
