...
| Groq `qwen/qwen3.6-27b` — agen probe DIPROVISION, tertahan rate limit free tier (D61) | Armada tidak punya agen groq (qwen-high/qwen-medium Test gagal "no agent provisioned"); agen probe `sem-workspaces-probe-groq-groq-qwen-qwen3-6-27b` dibuat 2026-09-06 di `workspaces/probe/groq` (chown 1000:1000 dulu — `agents.create` menulis AGENTS.md dan gagal EACCES sebagai root) mengiklankan `off\|minimal\|low\|medium\|high`. Setelah itu Test menemukan agen, dispatch diterima, dan vonis jujur tiba setelah re-wait (D61): `FailoverError: API rate limit reached` — groq free tier sedang terkena batas. Bukan bug controller; menunggu jendela reset (per-menit + harian, D59) atau paket berbayar |
| Suite controller | **436 test**, semua lulus, tanpa dependensi runtime |
