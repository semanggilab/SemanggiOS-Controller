  ...
  async function ensureProbeAgent(live, { provider, model }) {
    ...
    try {
      ...
      try {
        ...
        // Dua kunci keadaan gateway 2026.8 yang sama dengan pemangkas D81
        // (sandbox-provision createOne), kini juga di jalur probe — tanpa ini
        // Test Connection qwen-low terkunci selamanya pada satu workspace
        // ber-state legacy (terukur 2026-09-11: "Legacy workspace setup state
        // requires migration for …/workspaces/probe/groq"). SATU percobaan
        // ulang: pindahkan workspace, atau variasikan nama yang terkunci.
        const msg = String(err.gatewayError?.message ?? err.message);
        if (/Legacy workspace setup state/.test(msg)) {
          workspace = `${workspace}-r${crypto.randomUUID().slice(0, 8)}`.toLowerCase();
          log.warn("brain.test-provision-relocate", { probeName, workspace, full });
          created = await controller.runtime.createProbeAgent({ name: probeName, workspace, model: full });
        } else if (/deletion cleanup is still pending/i.test(msg)) {
